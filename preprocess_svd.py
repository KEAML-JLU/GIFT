...
def make_node2id_eng_text(dataset_name, remove_StopWord=False):
    ...
    stop_word.add('')
    os.makedirs(f'../data_svd/{dataset_name}_data', exist_ok=True)
    print(dataset_name)
    print("complete!")
    ...
    json.dump([adj_ent_index, ent_mapping],
              open('../data_svd/{}_data/index_and_mapping.json'.format(dataset_name), 'w'), ensure_ascii=False)
    ent_emb = []
    ...
    pkl.dump(np.array(ent_emb), open('../data_svd/{}_data/entity_emb.pkl'.format(dataset_name), 'wb'))
    pkl.dump(adj_ent, open('../data_svd/{}_data/adj_query2entity.pkl'.format(dataset_name), 'wb'))
    ...
    adj_tag = PMI(tag_list, tags_mapping, window_size=5, sparse=True)
    pkl.dump(adj_query2tag, open('../data_svd/{}_data/adj_query2tag.pkl'.format(dataset_name), 'wb'))
    pkl.dump(adj_tag, open('../data_svd/{}_data/adj_tag.pkl'.format(dataset_name), 'wb'))
    ...
    pkl.dump(adj_query2word, open('../data_svd/{}_data/adj_query2word.pkl'.format(dataset_name), 'wb'))
    pkl.dump(adj_word, open('../data_svd/{}_data/adj_word.pkl'.format(dataset_name), 'wb'))
    json.dump(train_idx, open('../data_svd/{}_data/train_idx.json'.format(dataset_name), 'w'), ensure_ascii=False)
    json.dump(test_idx, open('../data_svd/{}_data/test_idx.json'.format(dataset_name), 'w'), ensure_ascii=False)

    label_map = {value: i for i, value in enumerate(set(labels))}
    json.dump([label_map[label] for label in labels], open('../data_svd/{}_data/labels.json'.format(dataset_name), 'w'),
              ensure_ascii=False)
    json.dump(query_nodes, open('../data_svd/{}_data/query_id2_list.json'.format(dataset_name), 'w'),
              ensure_ascii=False)
    json.dump(tag_nodes, open('../data_svd/{}_data/tag_id2_list.json'.format(dataset_name), 'w'), ensure_ascii=False)
    json.dump(entity_nodes, open('../data_svd/{}_data/entity_id2_list.json'.format(dataset_name), 'w'),
              ensure_ascii=False)
    json.dump(word_nodes, open('../data_svd/{}_data/word_id2_list.json'.format(dataset_name), 'w'), ensure_ascii=False)

    glove_emb = pkl.load(open('./pretrained_emb/old_glove_6B/embedding_glove.p', 'rb')) # modify
    ...
    pkl.dump(np.array(embs, dtype=np.float64), open('../data_svd/{}_data/word_emb.pkl'.format(dataset_name), 'wb'))
...
if dataset_name in ['mr', 'snippets', 'tagmynews', 'StackOverflow']:
